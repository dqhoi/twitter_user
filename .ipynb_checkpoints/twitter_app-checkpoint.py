...
class MyStream(StreamingClient): # class ke thua StreamingClient tu Twitter xu ly, tim kiem du lieu theo mong muon.
    ...
    def __init__(self, c_socket, bearer_token): # Khởi tạo đối tượng Mystream cùng với key
        super().__init__(bearer_token=bearer_token)
        self.client_socket = c_socket
      
    def on_data(self, data): # Được gọi khi dữ liệu thô được nhận từ luồng. Gửi dữ liệu đến máy khách.
        try:
    ...
    def on_connect(self): # Được gọi sau khi kêt nối thành công đến Twitter APIs
        print("You are now connected to the streaming API.")
        return True

    def on_tweet(self, tweet): # được gọi khi nhận 1 tweet
        print('/ontweet')
        print(f"{tweet.id} {tweet.created_at} ({tweet.author_id}): {tweet.text}")
        print('ontweet/')
        print()
    
    def on_error(self, status_code): # được gọi khi có lỗi xảy ra
        print("Status Code: " + str(status_code))
        print("Error: " + repr(status_code))

    def disconnect(self): # dừng luồng thực thi 
        return super().disconnect()

    def on_disconnect(self): # được gọi sau khi dừng luồng
        print("Disconnect to the Stream")
        return super().on_disconnect()

    def filter(self, *, threaded=True, **params): # thực thi luồng tìm kiếm dữ liệu các tweets với các luật filter đưa vào
...
class Init: 
    ...
    def sendData(self): # Tìm kiếm thông tin các tweet thông qua hàm filter của class Mystream
        try:
...
class Main: # khoi tao cong dich vu theo mo hinh client server gui du lieu nhan duoc tu twitter den spark
    host = "localhost"      # địa chỉ 127.0.0.1
    port = 5557                 # cổng dịch vụ 
    def __init__(self):
        self.s = socket.socket()         # Tạo đối tượng socket
        self.s.bind((self.host, self.port))        # Liên kết với cổng
        print("Listening on port: %s" % str(self.port))
        self.s.listen(5)                 # Đợi kết nối từ máy khách( ở đây là spark)
        self.c_socket, self.addr = self.s.accept()        # Khởi tạo thành công với máy khách
